...
async function login(req, res) {
  ...
  if (!usuario.activo) {
    return res.status(403).json({ error: 'Cuenta bloqueada por seguridad. Revisá tu email.' });
  }
  ...
  if (!coincide) {
    ...
    if (actualizado.intentos_fallidos >= LIMITE_INTENTOS_FALLIDOS) {
      ...
      await enviarEmailIntentosFallidos({
        email: usuario.email,
      ...
      return res.status(401).json({
        error: 'Demasiados intentos fallidos. Te enviamos un email para verificar tu cuenta.',
      });
    }
...
async function confirmarActividad(req, res) {
  ...
  UsuarioModel.resetearIntentosFallidos(usuario.id_usuario);
  res.send('Gracias por confirmar. Tu cuenta sigue activa y el contador de intentos se reinició.');
}
...
async function reportarFraude(req, res) {
  ...
  res.send(
    'Tu cuenta fue bloqueada por seguridad. Para reactivarla, cambiá tu contraseña ' +
    'desde la opción "¿Olvidaste tu contraseña?" en el login.'
  );
}
...
async function pedirCambioContrasena(req, res) {
  ...
  // Nota: respondemos el mismo mensaje exista o no el usuario, para no
  // revelar qué emails están registrados en el sistema (evita enumeración).
  if (usuario) {
    const token = crypto.randomBytes(32).toString('hex');
    const expiracion = new Date(Date.now() + 60 * 60 * 1000).toISOString();
    UsuarioModel.guardarTokenSeguridad(usuario.id_usuario, token, expiracion);

    await solicitarCambioContrasena({
      email: usuario.email,
      nombre: usuario.nombre,
      token,
    });
